...
class JtagTransaction(UVMSequenceItem):
    ...
    def __init__(self, name="jtag_tr"):
        ...
        # 要移入 DR 的数据（48 位，LSB first）
        self.dr_in: int = 0
        # 从 TDO 移出的 DR 数据（采样结果）
        self.dr_out: int = 0
        # DR 有效位宽（IDCODE=32, BYPASS=1, DBGACC=48）
        self.dr_bits: int = 48
